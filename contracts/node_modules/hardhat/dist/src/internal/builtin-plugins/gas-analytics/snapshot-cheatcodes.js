...
import { FileNotFoundError, readdir, readdirOrEmpty, readJsonFile, remove, writeJsonFile, } from "@nomicfoundation/hardhat-utils/fs";
...
import { formatSectionHeader, getUserFqn, isWithinTolerance, } from "./helpers/utils.js";
...
export function getSnapshotCheatcodesPath(basePath, filename) {
 ...
 */
export function sanitizeSnapshotCheatcodes(snapshotCheatcodes) {
    const sanitizedSnapshotCheatcodes = new Map();
    const originalBySanitized = new Map();
    const renamedGroups = [];
    for (const [original, entries] of snapshotCheatcodes) {
        ...
        if (previousOriginal !== undefined) {
            const [nameA, nameB] = previousOriginal < original
                ? [previousOriginal, original]
                : [original, previousOriginal];
            throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.SNAPSHOT_GROUP_NAME_COLLISION, { nameA, nameB, sanitized: sanitizedName });
        }
...
export function extractSnapshotCheatcodes(suiteResults) {
...
async function deleteOrphanedSnapshotFiles(snapshotsDir, currentGroups) {
    try {
        ...
        ensureError(error);
        throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.SNAPSHOT_WRITE_ERROR, { snapshotsPath: snapshotsDir, error: error.message }, error);
    }
}
export async function writeSnapshotCheatcodes(basePath, snapshotCheatcodes) {
    ...
    for (const [snapshotGroup, snapshot] of snapshotCheatcodes) {
        const snapshotCheatcodesPath = getSnapshotCheatcodesPath(basePath, `${snapshotGroup}.json`);
        ...
        try {
            ...
            ensureError(error);
            throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.SNAPSHOT_WRITE_ERROR, { snapshotsPath: snapshotCheatcodesPath, error: error.message }, error);
        }
    }
}
export async function readSnapshotCheatcodes(basePath) {
    const snapshots = new Map();
    const snapshotsDir = path.join(basePath, SNAPSHOT_CHEATCODES_DIR);
    let dirEntries;
    try {
        ...
        throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.SNAPSHOT_READ_ERROR, { snapshotsPath: snapshotsDir, error: error.message }, error);
    }
    for (const entry of dirEntries) {
        if (entry.endsWith(".json")) {
            ...
            try {
                ...
                ensureError(error);
                throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.SNAPSHOT_READ_ERROR, { snapshotsPath: snapshotCheatcodesPath, error: error.message }, error);
            }
            if (!isObject(parsedSnapshot)) {
                throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.SNAPSHOT_READ_ERROR, {
...
export function stringifySnapshotCheatcodes(snapshots) {
    const lines = [];
...
export function compareSnapshotCheatcodes(previousSnapshotsMap, currentSnapshotsMap, tolerance) {
    const added = [];
    const removed = [];
    const changed = [];
    const tolerated = [];
    const seenPreviousEntries = new Set();
        ...
        for (const [name, currentEntry] of Object.entries(currentSnapshots)) {
            ...
            if (previousSnapshots === undefined ||
                ...
                if (previousValue !== currentEntry.value) {
                    ...
                    if (tolerance > 0 &&
                        isWithinTolerance(Number(previousValue), Number(currentEntry.value), tolerance)) {
    ...
    const sortByKey = (a, b) => `${a.group}#${a.name}`.localeCompare(`${b.group}#${b.name}`);
...
export async function checkSnapshotCheatcodes(basePath, suiteResults, tolerance) {
    ...
    const comparison = compareSnapshotCheatcodes(previousSnapshotCheatcodes, snapshotCheatcodes, tolerance);
    return {
...
export function logSnapshotCheatcodesSection(result, logger = console.log, isFiltered = false) {
    ...
    if (noBaseline) {
        logger(styleText("yellow", "Snapshot cheatcodes: no snapshots found. Run your tests with --snapshot to create one."));
        logger();
        return;
    }
    logger(formatSectionHeader("Snapshot cheatcodes", {
        changedLength,
        addedLength,
        removedLength,
    }));
    ...
    if (hasAdded) {
        logger();
        logger(`  ${comparison.added.length} snapshot(s) produced by this run are not in the snapshot:`);
    ...
    if (hasRemoved) {
        logger();
        logger(`  ${comparison.removed.length} stored snapshot(s) were not produced by this run:`);
...
export function printSnapshotCheatcodeChanges(changes, logger = console.log) {
    for (let i = 0; i < changes.length; i++) {
        ...
        const formattedGasChange = diff < 0 ? styleText("green", gasChange) : styleText("red", gasChange);
...
export function logSnapshotRenameWarnings(renamedGroups, logger = console.log) {
    if (renamedGroups.length === 0) {
        return;
    }
    logger(styleText("yellow", `Renamed ${renamedGroups.length} snapshot group name(s) for safe filesystem use:`));
    for (const { original, sanitized } of renamedGroups) {
        logger(styleText("yellow", `  "${original}" → "${sanitized}"`));
    }
    logger(styleText("yellow", "If you'd like the on-disk filename(s) to match exactly, consider renaming the group(s) in Solidity."));
    logger();
}
//# sourceMappingURL=snapshot-cheatcodes.js.map
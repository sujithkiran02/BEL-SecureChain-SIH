...
const runSolidityTests = async (args, hre, runSuper) => {
    ...
    if (args.tolerance < 0 || !Number.isFinite(args.tolerance)) {
        throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.INVALID_SNAPSHOT_TOLERANCE, { value: args.tolerance });
    }
    if (args.tolerance !== 0 && !args.snapshotCheck) {
        throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.SNAPSHOT_TOLERANCE_REQUIRES_CHECK);
    }
    ...
    if (args.snapshot) {
        ...
        const snapshotCheckResult = await handleSnapshotCheck(rootPath, suiteResults, args.tolerance);
...
export async function handleSnapshotCheck(basePath, suiteResults, tolerance) {
    const functionGasSnapshotsCheck = await checkFunctionGasSnapshots(basePath, suiteResults, tolerance);
    const snapshotCheatcodesCheck = await checkSnapshotCheatcodes(basePath, suiteResults, tolerance);
    return {

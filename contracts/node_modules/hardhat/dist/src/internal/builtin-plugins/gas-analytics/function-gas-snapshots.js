...
export function compareFunctionGasSnapshots(previousSnapshots, currentSnapshots, tolerance) {
...
export async function checkFunctionGasSnapshots(basePath, suiteResults, tolerance) {
    ...
    const comparison = compareFunctionGasSnapshots(previousFunctionGasSnapshots, functionGasSnapshots, tolerance);
    return {
...
export function logFunctionGasSnapshotsSection(result, logger = console.log, isFiltered = false) {
    ...
    if (noBaseline) {
        logger(styleText("yellow", "Function gas snapshots: no snapshot found. Run your tests with --snapshot to create one."));
        logger();
        return;
    }
    ...
    if (hasAdded) {
        logger();
        logger(`  ${comparison.added.length} function(s) produced by this run are not in the snapshot:`);
    ...
    if (hasRemoved) {
        logger();
        logger(`  ${comparison.removed.length} stored function(s) were not produced by this run:`);
...
export function printFunctionGasSnapshotChanges(changes, logger = console.log) {
    for (let i = 0; i < changes.length; i++) {
        ...
        logger(styleText("grey", `    Actual (${label}):   ${change.actual} (`) +
            formattedGasChange +
            styleText("grey", ")"));
        if (!isLast) {
            logger();

...
import { exists, TrueCasePathResolver, } from "@nomicfoundation/hardhat-utils/fs";
import { AsyncMutex } from "@nomicfoundation/hardhat-utils/synchronization";
import { analyze } from "@nomicfoundation/solidity-analyzer";
import { ImportResolutionErrorType, RootResolutionErrorType, } from "../../../../../types/solidity/errors.js";
import { ResolvedFileType } from "../../../../../types/solidity/resolved-file.js";
import { parseNpmDirectImport } from "./npm-module-parsing.js";
import { isResolvedUserRemapping, RemappedNpmPackagesGraphImplementation, } from "./remapped-npm-packages-graph.js";
import { applyValidRemapping, formatRemapping } from "./remappings.js";
import { fsPathToSourceNamePath, sourceNamePathJoin, sourceNamePathToFsPath, } from "./source-name-utils.js";
import { UserRemappingType } from "./types.js";
import { PathValidationErrorType, resolveSubpathWithPackageExports, validateFsPath, } from "./utils.js";
const NPM_PACKAGES_WITH_SIMULATED_PACKAGE_EXPORTS = new Set(["forge-std"]);
function isInputSourceNameWithinRoot(inputSourceName, inputSourceNameRoot) {
    return (inputSourceName === inputSourceNameRoot ||
        inputSourceName.startsWith(`${inputSourceNameRoot}/`));
}
export class ResolverImplementation {
    #projectRoot;
    #npmPackageGraph;
    #hhProjectPackage;
    #readUtf8File;
     ...
     */
    static async create(projectRoot, readUtf8File, remappingsReader) {
        const map = await RemappedNpmPackagesGraphImplementation.create(projectRoot, remappingsReader);
        return new ResolverImplementation(projectRoot, map, readUtf8File);
    }
    constructor(projectRoot, npmPackagesGraph, readUtf8File) {
    ...
    async #resolveProjectFile(absoluteFilePath) {
        ...
        let inputSourceName = sourceNamePathJoin(this.#hhProjectPackage.inputSourceNameRoot, fsPathToSourceNamePath(relativeFilePath));
        ...
        if (existing !== undefined) {
            ...
            const existingProjectResolvedFile = existing;
            return {
                success: true,
        ...
        const pathValidation = await validateFsPath(this.#trueCasePathResolver, this.#projectRoot, relativeFilePath);
        ...
        if (pathValidation.success === false) {
            ...
            inputSourceName = sourceNamePathJoin(this.#hhProjectPackage.inputSourceNameRoot, fsPathToSourceNamePath(realCasingRelativePath));
        }
        // Maybe it was already resolved, so we need to check with the right
        // casing
        const resolvedWithTheRightCasing = this.#resolvedFileByInputSourceName.get(inputSourceName);
        if (resolvedWithTheRightCasing !== undefined) {
            ...
            /* eslint-disable-next-line @typescript-eslint/consistent-type-assertions
              ...
              source name */
            resolvedWithTheRightCasing;
            return {
                success: true,
                value: resolvedWithTheRightCasingProjectResolvedFile,
            };
        }
        const fsPathWithTheRightCasing = path.join(this.#projectRoot, realCasingRelativePath);
        ...
        const resolvedFileAsProjectFile = resolvedFile;
        return {
            success: true,
    ...
    toJSON() {
        return {
            resolvedFileBySourceName: Object.fromEntries(this.#resolvedFileByInputSourceName.entries()),
    ...
    async #resolveNpmDependencyFileAsRoot(npmModule) {
        ...
        if (result.success === false) {
            return {
                success: false,
                error: this.#importResolutionErrorToNpmRootResolutionError(npmModule, result.error),
            };
        }
        const resolvedFile = result.value.file;
        assertHardhatInvariant(result.value.remapping !== undefined, "We must have a remapping here, because we either resolved though a user remapping, or npm");
        ...
        if (resolvedFile.type !== ResolvedFileType.NPM_PACKAGE_FILE) {
            assertHardhatInvariant(remapping !== undefined, "Expected user remapping to be present if the import resolved to a local file");
            return {
                success: false,
                error: {
    ...
    async #resolveImport(from, importPath) {
        ...
        if (isRelativeImport) {
            ...
            if (!isInputSourceNameWithinRoot(directImport, from.package.inputSourceNameRoot)) {
                return {
                    success: false,
                    error: {
            ...
            if (isInputSourceNameWithinRoot(directImport, sourceNamePathJoin(from.package.inputSourceNameRoot, "node_modules"))) {
                return {
                    success: false,
                    error: {
        ...
        if (isRelativeImport) {
            ...
            if (npmResolutionResult.error.type ===
    ...
    async #resolveRelativeImport({ from, importPath, directImport, }) {
    ...
    async #resolveImportThroughNpm({ from, importPath, directImport, }) {
        ...
        if (dependencyResolution === undefined) {
            ...
            const foundryTomlPath = path.join(from.package.rootFsPath, "foundry.toml");
    ...
    async #commonImportResolution({ from, importPath, npmPackage, inputSourceName, relativeFsPathWithinPackage, subpath, userRemapping, generatedRemapping, packageExportsResolvedSubpath, }) {
        const pathValidation = await validateFsPath(this.#trueCasePathResolver, npmPackage.rootFsPath, relativeFsPathWithinPackage);
        if (pathValidation.success === false) {
            if (pathValidation.error.type === PathValidationErrorType.DOES_NOT_EXIST) {
                return {
                    success: false,

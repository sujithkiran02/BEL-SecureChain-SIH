 ...
 * reporter's design.
 */
export async function* testReporter(source, sourceNameToUserSourceName, verbosity, testSummaryIndex = 0, 
        ...
        for (const { testResult: failure, contractName } of failures) {
            ...
            switch (stackTrace?.kind) {
                ...
                case "UnexpectedError":
                    ...
                    yield* output(indenter.t `Stack Trace Warning: ${colorize("grey", stackTrace.errorMessage)}\n`);
                    break;
                case "UnsafeToReplay":
                    if (stackTrace.globalForkLatest === true) {
                        yield* output(indenter.t `Stack Trace Warning: ${colorize("grey", "The test is not safe to replay because a fork url without a fork block number was provided.")}\n`);
                        yield* output(indenter.t `${RERUN_VERBOSITY_HINT}\n`);
                    }
                    if (stackTrace.impureCheatcodes.length > 0) {
                        yield* output(indenter.t `Stack Trace Warning: ${colorize("grey", `The test is not safe to replay because it uses impure cheatcodes: ${stackTrace.impureCheatcodes.join(", ")}`)}\n`);
                        yield* output(indenter.t `${RERUN_VERBOSITY_HINT}\n`);
                    }
                    break;
            ...
            if (failure.counterexample !== undefined &&
                ...
                for (const counterexample of counterexamples) {
                    ...
                    for (const [key, value] of Object.entries(counterexample)) {
                        ...
                        yield* output(indenter.t `${colorize("grey", counterExampleDetails)}\n`);
                    }
                    indenter.dec();
                }
            }

...
import { opGenesisState, l1GenesisState, FsAccessPermission, CollectStackTraces, opHardforkFromString, l1HardforkFromString, } from "@nomicfoundation/edr";
import { toBigInt } from "@nomicfoundation/hardhat-utils/bigint";
import { hexStringToBytes } from "@nomicfoundation/hardhat-utils/hex";
import { ALWAYS_COLLECT_STACK_TRACES_VERBOSITY, OPTIMISM_CHAIN_TYPE, } from "../../constants.js";
...
export async function solidityTestConfigToSolidityTestRunnerConfigArgs({ chainType, projectRoot, hardfork, config, verbosity, observability, testPattern, excludeTestPattern, generateGasReport, eip712CanonicalTypes, testSourcePaths, }) {
    const fsPermissions = [
    ...
    const hexToBytes = (hex) => hex !== undefined ? hexStringToBytes(hex) : undefined;
    ...
    const resolvedHardfork = hardhatHardforkToEdrSpecId(resolvedHardforkName, chainType);
    ...
    const blockGasLimit = typeof config.blockGasLimit === "number" ||
        typeof config.blockGasLimit === "bigint"
        ? toBigInt(config.blockGasLimit)
        : undefined;
    const disableBlockGasLimit = blockGasLimit === undefined;
    const transactionGasCap = typeof config.transactionGasCap === "number" ||
    ...
    const shouldAlwaysCollectStackTraces = verbosity >= ALWAYS_COLLECT_STACK_TRACES_VERBOSITY;
    return {
        projectRoot,
        ...
        excludeTestPattern: excludeTestPattern === "" ? undefined : excludeTestPattern,
        includeTraces,
        blockGasLimit,
 ...
 */
export async function writeTestRunOutput(runStream, reporterStream, output) {
    ...
    let reporterStreamError;
    reporterStream.on("error", (error) => {
...
export function warnDeprecatedTestFail(artifact, sourceNameToUserSourceName) {
    const abi = JSON.parse(artifact.contract.abi);

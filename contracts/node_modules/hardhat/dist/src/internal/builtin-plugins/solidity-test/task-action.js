...
import { isTestSuiteArtifact, warnDeprecatedTestFail, solidityTestConfigToSolidityTestRunnerConfigArgs, writeTestRunOutput, } from "./helpers.js";
import { testReporter } from "./reporter.js";
import { run } from "./runner.js";
import { DEFAULT_TEST_PROFILE } from "./test-profiles.js";
const runSolidityTests = async ({ testFiles, chainType, grep, grepExclude, noCompile, testSummaryIndex }, hre) => {
    ...
    if (hre.config.solidity.splitTestsCompilation) {
        ...
        if (noCompile === true && testFiles.length > 0) {
            ...
            if (notCompiledFiles.length > 0) {
                throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS
    ...
    const testSourcePaths = Object.fromEntries(testSuiteArtifacts.map(({ userSourceName, edrArtifact }) => [
        edrArtifact.id.source,
    ...
    const { eip712Types, ...solidityTestConfig } = hre.config.test.solidity.profiles[DEFAULT_TEST_PROFILE];
    ...
    const eip712CanonicalTypes = await collectEip712CanonicalTypes(allBuildInfosAndOutputs, sourceNameToUserSourceName, eip712Types);
    ...
    const testRunError = await writeTestRunOutput(runStream, testReporterStream, createNonClosingWriter(process.stdout));

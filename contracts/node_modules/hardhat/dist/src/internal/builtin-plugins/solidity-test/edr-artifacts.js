...
export async function getBuildInfosAndOutputs(artifactManager) {
    ...
    return await Promise.all(Array.from(buildInfoIds).map(async (buildInfoId) => {
        ...
        assertHardhatInvariant(buildInfoPath !== undefined, "buildInfoPath should not be undefined");
        assertHardhatInvariant(buildInfoOutputPath !== undefined, "buildInfoOutputPath should not be undefined");
...
export async function buildEdrArtifactsWithMetadata(artifactManager) {
    ...
    const buildInfoIds = Array.from(new Set(artifacts.map((artifact) => {
        assertHardhatInvariant(artifact.buildInfoId !== undefined, `buildInfoId should not be undefined for artifact: ${displayArtifactForError(artifact)}`);
        return artifact.buildInfoId;
    })));
    const solcVersionsArray = buildInfoIds
        .map((buildInfoId) => {
        ...
        assertHardhatInvariant(match.groups !== undefined, "The match must have groups");
        const solcShortVersion = `${match.groups.major}.${match.groups.minor}.${match.groups.patch}`;
        const result = [buildInfoId, solcShortVersion];
        return result;
    })
    ...
    return artifacts.map((artifact) => {
        ...
        assertHardhatInvariant(solcVersion !== undefined, `solcVersion should not be undefined for artifact: ${displayArtifactForError(artifact)}`);
        const id = {
...
function displayArtifactForError(artifact) {
    return `'${artifact.contractName}' in '${artifact.sourceName}'`;
}
//# sourceMappingURL=edr-artifacts.js.map
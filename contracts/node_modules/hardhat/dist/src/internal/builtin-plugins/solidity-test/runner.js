 ...
 */
export function run(chainType, artifacts, testSuiteIds, testRunnerConfig, tracingConfig, sourceNameToUserSourceName) {
    ...
    const remainingSuites = new Set(testSuiteIds.map((id) => formatArtifactId(id, sourceNameToUserSourceName)));
    ...
    void (async () => {
        try {
            ...
            const solidityTestResult = await edrContext.runSolidityTests(hardhatChainTypeToEdrChainType(chainType), artifacts, testSuiteIds, testRunnerConfig, tracingConfig, (suiteResult) => {
                stream.push({
                    type: "suite:done",
                    data: suiteResult,
                });
                remainingSuites.delete(formatArtifactId(suiteResult.id, sourceNameToUserSourceName));
            ...
            if (hasInlineConfigErrors(error)) {
                stream.destroy(new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.INVALID_INLINE_CONFIG, {
                    errors: formatInlineConfigErrors(error.inlineConfigErrors, sourceNameToUserSourceName),
                }));
                return;
            }
            stream.destroy(new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS
 ...
 */
function hasInlineConfigErrors(error) {
    return ("inlineConfigErrors" in error && Array.isArray(error.inlineConfigErrors));
}

import { getUnprefixedHexString, isHexString, } from "@nomicfoundation/hardhat-utils/hex";
import { isObject } from "@nomicfoundation/hardhat-utils/lang";
import { conditionalUnionType, configurationVariableSchema, sensitiveStringSchema, sensitiveUrlSchema, unionType, validateUserConfigZodType, } from "@nomicfoundation/hardhat-zod-utils";
import { z } from "zod";
import { GENERIC_CHAIN_TYPE, L1_CHAIN_TYPE, OPTIMISM_CHAIN_TYPE, } from "../../constants.js";
import { hardforkGte, L1HardforkName, isValidHardforkName, getHardforks, getCurrentHardfork, } from "./edr/types/hardfork.js";
const nonnegativeNumberSchema = z.number().nonnegative();
const nonnegativeIntSchema = z.number().int().nonnegative();
const nonnegativeBigIntSchema = z.bigint().nonnegative();
const blockNumberSchema = nonnegativeIntSchema;
const forkingBlockNumberSchema = unionType([z.number().int().nonnegative().safe(), z.bigint().nonnegative()], "Expected a nonnegative safe int or a nonnegative bigint");
...
const hardforkHistoryUserConfigSchema = z.record(conditionalUnionType([
    [
...
unionType([chainIdSchema, z.string()], "Expected a number or a string"), chainDescriptorUserConfigSchema)
    ...
    if (chainDescriptors !== undefined) {
        Object.entries(chainDescriptors).forEach(([chainId, chainDescriptor]) => {
            ...
            let previousValue = 0;
            Object.entries(chainDescriptor.hardforkHistory).forEach(([name, activation]) => {
...
const accountsPrivateKeyUserConfigSchema = unionType([
    ...
    z
        .string()
        .refine((val) => isHexString(val) && getUnprefixedHexString(val).length === 64),
...
const gasUnitUserConfigSchema = unionType([nonnegativeIntSchema.safe(), nonnegativeBigIntSchema], "Expected a positive safe int or a positive bigint");
const gasLimitWithDisableUserConfigSchema = unionType([gasUnitUserConfigSchema, z.literal(false)], "Expected `false`, a positive safe int, or a positive bigint");
const gasUserConfigSchema = unionType([z.literal("auto"), gasUnitUserConfigSchema], "Expected 'auto', a positive safe int, or positive bigint");
...
const accountBalanceUserConfigSchema = unionType([z.string(), nonnegativeBigIntSchema], "Expected a string or a positive bigint");
...
const edrNetworkMempoolUserConfigSchema = z.object({
    order: z.optional(unionType([z.literal("fifo"), z.literal("priority")], "Expected 'fifo' or 'priority'")),
});
const edrNetworkMiningUserConfigSchema = z.object({
    auto: z.optional(z.boolean()),
    interval: z.optional(unionType([
...
const edrNetworkUserConfigSchema = z.object({
    ...
    initialDate: z.optional(unionType([z.string(), z.instanceof(Date)], "Expected a string or a Date").refine((val) => {
...
function refineEdrNetworkUserConfig({ defaultChainType = GENERIC_CHAIN_TYPE, networks = {} }, ctx) {
    ...
    for (const [networkName, network] of Object.entries(networks)) {
        if (network.type === "edr-simulated") {
            const { chainType, hardfork, minGasPrice, initialBaseFeePerGas } = network;
            ...
            if (hardfork !== undefined &&
                ...
                ctx.addIssue({
                    ...
                    message: `Invalid hardfork name ${hardfork} for chainType ${resolvedChainType}. Expected ${getHardforks(resolvedChainType).join(" | ")}.`,
                });
            }
            const resolvedHardfork = hardfork ?? getCurrentHardfork(resolvedChainType);
            if (resolvedChainType === L1_CHAIN_TYPE) {
                if (hardforkGte(resolvedHardfork, L1HardforkName.LONDON, resolvedChainType)) {
                    if (minGasPrice !== undefined) {
                        ctx.addIssue({
                            code: z.ZodIssueCode.custom,
                            path: ["networks", networkName, "minGasPrice"],
                            message: "minGasPrice is not valid for networks with EIP-1559. Try an older hardfork or remove it.",
                        });
                    }
                }
                else {
                    if (initialBaseFeePerGas !== undefined) {
                        ctx.addIssue({
                            ...
                            message: "initialBaseFeePerGas is only valid for networks with EIP-1559. Try a newer hardfork or remove it.",
                        });
                    }
                }
            }
            ...
            if ((typeof interval === "number" && interval !== 0) ||
                ...
                const minInterval = typeof interval === "number" ? interval : Math.min(...interval);
                if (minInterval < 1000 &&
...
export async function validateNetworkUserConfig(userConfig) {
...
export function isHttpNetworkHdAccountsUserConfig(accounts) {
    return isObject(accounts);
}
export function isHttpNetworkHdAccountsConfig(accounts) {
    return (isObject(accounts) &&
...
export function isEdrNetworkHdAccountsConfig(accounts) {
    return (isObject(accounts) &&
        "mnemonic" in accounts &&
...
export function isEdrNetworkForkingConfig(forking) {
    return (isObject(forking) &&
        "enabled" in forking &&
        "url" in forking &&
        "cacheDir" in forking);
}
export function isEdrNetworkMiningConfig(mining) {
    return (isObject(mining) &&
        "auto" in mining &&
        "interval" in mining &&
        "mempool" in mining);
}

...
import { parseBuiltinGlobalOptions, parseGlobalOptions, parseRawArguments, parseTask, parseTaskArguments, } from "./parser.js";
...
import { sendErrorTelemetry, setCliHardhatConfigPath, } from "./telemetry/error-reporter/reporter.js";
...
export async function main(rawArguments, options = {}) {
    ...
    try {
        ...
        if (builtinGlobalOptions.init) {
            const { initHardhat, initHardhat3NonInteractive, printTemplatesList } = await import("./init/init.js");
            let templateName;
            ...
            for (let i = 0; i < cliArguments.length; i++) {
                ...
                if (cliArguments[i] === "--template") {
                    ...
                    if (templateName !== undefined) {
                        throw new HardhatError(HardhatError.ERRORS.CORE.ARGUMENTS.DUPLICATED_NAME, { name: "--template" });
                    }
                    if (listTemplates) {
                        throw new HardhatError(HardhatError.ERRORS.CORE.ARGUMENTS
                    ...
                    if (usedCliArguments[i + 1] === undefined ||
                        ...
                        throw new HardhatError(HardhatError.ERRORS.CORE.ARGUMENTS.MISSING_VALUE_FOR_ARGUMENT, { argument: "--template" });
                    }
        ...
        const taskOrId = parseTask(cliArguments, usedCliArguments, hre.tasks.rootTasks);
        ...
        const deprecatedConnectWasCalled = Boolean("wasConnectCalled" in hre.network &&
        ...
        if (deprecatedConnectWasCalled) {
            console.warn("WARNING: hre.network.connect() is deprecated and will be removed in a future version. " +
